...
This application implements a two-step verification process for sending funds. This ensures that transactions are not sent accidentally.

### How it works:

1.  **Initiate Transfer:** A user initiates a transfer by calling the `initiate_transfer` function with the receiver's address and the amount.
2.  **Confirm Transfer:** The owner of the contract must then call the `confirm_transfer` function to approve and execute the transaction.
3.  **Cancel Transfer:** The owner can also call the `cancel_transfer` function to cancel a pending transaction.
...
By default the template creates a single `HelloWorld` contract under safesend_contracts folder in the `smart_contracts` directory. To add a new contract:
